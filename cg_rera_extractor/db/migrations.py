    ...
    conn.execute(
        text(
            ...
            SELECT
                ...
                p.full_address,
                COALESCE(p.latitude, cl.lat) AS lat,
                COALESCE(p.longitude, cl.lon) AS lon,
                COALESCE(p.geo_source, cl.source_type) AS geo_source,
                COALESCE(p.geo_precision, cl.precision_level) AS geo_precision,
                COALESCE(p.geo_confidence, cl.confidence_score) AS geo_confidence,
